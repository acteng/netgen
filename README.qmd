...
There are many sources of OD data, see the documentation in the [odgen.qmd](data-raw/odgen.qmd) for a way to generate synthetic OD data that we'll use in the reproducible code below.

# Setup

For reproduducibility, we have setup a Docker container with all the dependencies needed to run the code below.
The easiest way to run the code and develop new code in this repo may be with GitHub Codespaces which can be accessed via [github.com/codespaces/new](https://github.com/codespaces/new?hide_repo_select=true&ref=24-package-all-software-in-single-image&repo=826170244&skip_quickstart=true&machine=standardLinux32gb&devcontainer_path=.devcontainer%2Fdevcontainer.json&geo=EuropeWest).
...
After getting the software installed, load packages we'll use as follows:

```{r}
#| eval: false
#| echo: false
# Install gh if not already installed:
system('if ! command -v gh &> /dev/null
then
  echo "gh could not be found"
  echo "Installing gh..."
  curl -sL https://cli.github.com/packages/githubcli-archive-keyring.gpg | gpg --dearmor -o /usr/share/keyrings/githubcli-archive-keyring.gpg
...
sudo ln -s ~/.cargo/bin/od2net /usr/bin/od2net
```
