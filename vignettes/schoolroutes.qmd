...
rnet_walking_cycling_stratford = rnet_walking_cycling[stratford_way_buffer_2km, op = st_within]
table(rnet_walking_cycling_stratford$mode)

# Plot with tmap and facets:
m = tm_shape(rnet_walking_cycling_stratford) +
  tm_lines(
    col = "lts",
    palette = "-viridis",
    lwd = "count",
...
# Debugging with reprex:
# https://github.com/r-tmap/tmap/issues/983
reprex::reprex({
remotes::install_dev("tmap")
library(tmap)
# Works:
tm_shape(rivers) +
	tm_lines(lwd = "strokelwd", col = "type", lwd.legend = NULL)
# Fails when there is a scale arg:
tm_shape(rivers) +
	tm_lines(lwd = "strokelwd", col = "type", lwd.legend = NULL, scale = 9)
})
