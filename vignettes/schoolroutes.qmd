...
The figure below highlights this by showing the routes generated by the SchoolRoutes project in the vicinity of Stratford Way, next to Huntingdon School, with colours representing selected variables.
...
The output dataset can be visualised with well-known data science tools, such as the `ggplot2` R package.
 <!-- as follows shown in  -->
...
```{r rnet-godutch-top5, tab.cap="Top 5 routes with the highest potential distance cycled each day for travel to school under the Government Target scenario."}
rnet_godutch_top_5 |>
  sf::st_drop_geometry() |>
...
The total Euclidean distance travelled according to the OD data is `r round(distance_euclidean_total / 1000) |> format(big.mark = ",")` km, with a weighted mean distance of `r round(wm, 1) |> format(big.mark = ",")` km.
...
Assuming a trip diversion factor of 1.4 and that on average 60% of the trips are made by cycling under the Go Dutch scenario for the short distances travelled to school, we can estimate the total number distance travelled by bike to school on a typical school day as `r round(total_euclidean_cycled / 1000) |> format(big.mark = ",")` km.
...
These numbers make sense and align well with the peer-reviewed results of the Propensity to Cycle Tool project [@goodman_scenarios_2019].

# Conclusions and next steps

The report has outlined a new approach to estimating the potential for active travel to school, based on disaggregated OD data and route network outputs, building on but going beyond the Propensity to Cycle Tool project.
...
-  Detailed quantitative comparison of results for York with Propensity to Cycle Tool outputs (2 days)
-  Validating the results against ground truth data for a single local authority (5 days)
-  Refining the parameters in the walking uptake (and potentially cycling uptake) functions (5-10 days)
-  Sense-checking results based on local knowledge with input from inspectors and/or local authority staff (5 days)
- Running the tool for every local authority in England (10 days)
- Development of front-end tools for visualising the results (10 days)
...
- Training local authority staff to use the tool (5 days)
- Integration into the browse tool in the [`atip`](https://github.com/acteng/atip)
-  Writing up the results in a peer-reviewed journal article (10 days)

In total, the project would require around 60 days of work to complete, with the potential to have a substantial impact on active travel to school in England.
