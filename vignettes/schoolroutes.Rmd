...
The total distance travelled according to the OD data is `r round(distance_euclidean_total / 1000)` km.
...
The weighted mean distance travelled is `r round(wm, 1)` km.
...
Assuming a trip diversion factor of 1.3 and that 50% of the trips are made by cycling under the Government Target scenario for the short distances travelled to school, we can estimate the total number of trips and distance travelled, as `r round(total_euclidean_cycled / 1000)` km.
...
The equivalent for the route network output under the Go Dutch scenario is `r round(total_rnet / 1e3)` km.


Let's compare with the number who cycle under the less ambitious Government Target scenario:
The total distance travelled under the Government Target scenario is `r round(total_rnet_govtarget / 1e3)` km.
