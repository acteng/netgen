...
These numbers make sense and align well with the peer-reviewed results of the Propensity to Cycle Tool project.

A visual approach to quality assurance is to compare the results with the Propensity to Cycle Tool outputs, which are available for York, as illustrated below.

```{r pct-comparison, fig.cap="Comparison of the SchoolRoutes and Propensity to Cycle Tool outputs for York.", out.width="50%", fig.show="hold"}
...
Not that while there are differences in the results, the SchoolRoutes outputs presented above are based on synthetic OD data.
...
It is clear from the SchoolRoutes outputs, on the right in the figure above, that the networks it generates are much denser: most of the roads around the school are highlighted, rather than just a few key routes in the PCT.
