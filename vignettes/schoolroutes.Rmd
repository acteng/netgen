...
```{r, fig.cap="Uptake functions for walking and cycling to school, based on distance and the Go Dutch scenario.", fig.height=3}

```

The uptake levels produced by Go Dutch scenarios, for commuting and travel to school are illustrated in the graph below:
...
p_godutch = uptake_pct_godutch_2020(
    distance = uptake_df$distances,
    gradient = uptake_df$hilliness
    )
uptake_df = rbind(
  cbind(uptake_df, scenario = "Go Dutch (school)", pcycle = p_godutch_school),
  cbind(uptake_df, scenario = "Go Dutch (commute)", pcycle = p_godutch)
)
library(ggplot2)
ggplot(uptake_df) +
  geom_line(aes(
